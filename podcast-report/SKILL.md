...
# 3) 按 references/report-template.html 撰写报告

# 4) 归档 Obsidian 子文件夹 + 更新索引.md（硬性步骤，见下节，不可跳过）
```

**全流程四步，第 4 步是交付的一部分，不是可选项。**
...
**报告写完不等于交付完成——没进 Obsidian 并更新索引，等于没做。**

1. **目录**：`~/Obsidian/AI技术/播客学习报告/`（与「视频学习报告」并列）。
   可用环境变量 `OBSIDIAN_PODCAST_REPORT_DIR` 覆盖。

2. **必须按内容归入子文件夹**，不要平铺在根目录。已有分类：
   `AI模型/`、`AI产品/`。新分类只在内容确实不属于任何现有分类时才建，
   命名与视频报告分类保持同一套语感（AI模型 / AI落地 / AI产品 / Agent工程 / 编程效率 / 科普 …）。

3. **文件名 = 能一眼看出内容的核心命题**（中文，不用英文短名、不加日期前缀）。
   若单集标题是标题党或信息量过低，按实际内容改写——
   例：单集原名「腾讯、阿里、字节争夺打工人…」→ 归档名
   「大厂为何集体加码AI办公：模型、Harness与上下文的三层战争」。

4. **必须更新 `索引.md`**，每条含：
   - 总览表该分类的计数 +1
   - 分类区块里的双链条目：
     `[[播客学习报告/<分类>/<文件名>|<文件名>]]`
   - 四行信息：节目/主播/时长/发布日 · 一句话结论 · 关键落点（指向用户真实项目）· 转录规模
   - 「摄入状态」里补上由此产生的待办

5. **写完索引后自检一次双链**：`find` 确认报告文件真的躺在索引里写的那个子文件夹下。
   ⚠️ 真踩过的坑：索引写了 `AI模型/xxx` 但文件实际留在根目录 → Obsidian 双链全断，
   从索引点进去是空文件。**先建目录 / 移动文件，再写索引**，顺序别反。
