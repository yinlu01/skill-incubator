...
# -*- coding: utf-8 -*-
"""
环境自检：确认 bili-video-report 的全部依赖是否就位。
...
MODEL = os.environ.get("BILI_WHISPER_MODEL") or os.environ.get("XHS_WHISPER_MODEL") or "mlx-community/whisper-large-v3-turbo"
...
def c_bsk():
    p = _bsk_path()
    if not p:
        return False, "未找到（仅导 cookie 时必需），装 browser-skill 后应在 ~/.local/bin/bsk"
    return True, p
...
def main():
    ...
    check("yt-dlp（无需）", lambda: (True, "本 skill 不依赖 yt-dlp（实测 412，走 playurl+ffmpeg）"))
    print("-" * 56)
    if ok_all:
        print("全部就绪，可以跑完整流程。")
    else:
